...
const (
	defaultServiceTableFormat = "table {{.ID}}\t{{.Name}}\t{{.Mode}}\t{{.Replicas}}\t{{.Status}}\t{{.Image}}\t{{.Ports}}"

	serviceIDHeader  = "ID"
	modeHeader       = "MODE"
	replicasHeader   = "REPLICAS"
	waitStatusHeader = "STATUS"
)
...
func WaitListFormatWrite(ctx formatter.Context, services []swarm.Service, info map[string]WaitInfo) error {
	render := func(format func(subContext formatter.SubContext) error) error {
		for _, service := range services {
			serviceCtx := &serviceContext{
				service:  service,
				mode:     info[service.ID].Mode,
				replicas: info[service.ID].Replicas,
				expected: info[service.ID].Expected,
				running:  info[service.ID].Running,
			}
			if err := format(serviceCtx); err != nil {
				return err
			}
...
func (c *serviceContext) Status() string {
	if c.expected > c.running {
		return "Waiting"
	}
	return "Replicated"
}
